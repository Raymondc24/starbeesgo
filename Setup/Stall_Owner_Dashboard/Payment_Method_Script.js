document.addEventListener("DOMContentLoaded", () => {
    ...
    async function fetchAndRender(selectedCategory = null) {
        ...
        paymentMethods.forEach(({ id, data }) => {
            ...
            card.className = "payment-option payment-option-margin";
            card.innerHTML = `
                ${iconUrl ? `<img src="${iconUrl}" alt="icon" class="payment-icon">` : ""}
                <div class="payment-name">${data.name}</div>
                <div class="payment-btn-group">
                    <button class="select-payment-btn edit-btn" data-id="${id}">Edit</button>
                    <button class="select-payment-btn delete-btn" data-id="${id}">Delete</button>
                </div>
            `;
            card.querySelector(".edit-btn").addEventListener("click", () => editPaymentMethod(id));
            card.querySelector(".delete-btn").addEventListener("click", () => deletePaymentMethod(id));
